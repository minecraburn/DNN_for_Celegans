...
check2=2;
figure(1)
plot(cy1(winl:winr,check1),cy1(winl:winr,check2));
...
check2=2;
figure(2)
plot(cy2(winl2:winr2,check1),cy2(winl2:winr2,check2));
%% Sixth Block
...
% fig7a concise
% xsidel=0.68;xsider=0.9;ysidel=0.9;ysider=1.3;step=0.0025;
% pc=0;
% nea=4;neb=12;test_sig=2e-7;test_sig2=2e-6;
...
if re_cy1==1
    all_Sig=zeros(tfie+1,dim,dim);
    all_Sig1=zeros(tfie+1,dim,dim);
    all_Sig2=zeros(tfie+1,dim,dim);
    for i=1:1:tfie
        temp1=squeeze(Fi(i,:));
...
ylim([ysidel,ysider]);
% concise=1;
if have_tr==0 && concise==1
    hold on
%     plot3(read_cy1(3000:5000,nea),read_cy1(3000:5000,neb),10+0*read_cy1(3000:5000,1),'linewidth',0.1,'color','black');
%     plot3(read_cy2(6000:8000,nea),read_cy2(6000:8000,neb),10+0*read_cy2(6000:8000,1),'linewidth',0.1);
    xlim([0.68,0.9]);
    ylim([0.9,1.3]);
    set(gca,'ytick',[0.8,1,1.2]);
    set(gca,'xtick',[0.7,0.8,0.9]);
end
